...
// vector<int> homologySizes(complexes::Complex* COMPLEX, int dimension) {
...
// 	// Compute 
// 	vector<int> times = arithmetic::PHATPersistence(COMPLEX, FILTRATION, dimension/2);
// 	vector<int> sizes;
...
int main() {
	vector<int> corners(2,3);
	complexes::Cubical<ATEAMS::ff> CUBICAL(corners);

	Field F(SparseRREF::FIELD_Fp, 3);

	CUBICAL.constructBoundaryMatrices(F);
	cout << CUBICAL.size() << endl;
