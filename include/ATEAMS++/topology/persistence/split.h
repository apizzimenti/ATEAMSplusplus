...
#define ATEAMS_TOPOLOGY_PERSISTENCE_SPLIT_H
...
namespace ATEAMS::topology::persistence {
	 ...
	 * coefficients, where \f$p\f$ is prime. Computes persistence by performing
	 * @ref ATEAMS::topology::persistence::twist concurrently on the left
	 * and right halves of the matrix. Base implementation to which all overloads forward.
	 * 
	...
	/**
	 * @brief Computes the persistent homology of a complex over \f$\Z/p\Z\f$
	 * coefficients, where \f$p\f$ is prime. Overload for computing the persistent
	 * homology of a specific dimension.
	 * 
	 * @tparam RingLike A coefficient @ref Ring, like @ref Zp or @ref Q.
	 * 
	 * @param complex A (pointer to) a @ref ATEAMS::complexes::Complex with a flat boundary matrix.
	 ...
	 * coefficients, where \f$p\f$ is prime. Computes persistence by performing
	 * @ref ATEAMS::topology::persistence::twist concurrently on the left
	 * and right halves of the matrix. Convenience overload.
	 * 
	 * @tparam RingLike A coefficient @ref Ring, like @ref Zp or @ref Q.
	 * 
	 * @param complex A (pointer to) a @ref ATEAMS::complexes::Complex with a flat boundary matrix.
...
#include "ATEAMS++/topology/persistence/split.tpp"
