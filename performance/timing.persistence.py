...
for host in persistence.hosts:
	for computing in persistence.computing:
		data = persistence.data(host, computing)
...
overlaid = persistence.plots.timeByOverlapOverlaid


for host in persistence.hosts:
	parallel = persistence.data(host, "parallel")
	serial = persistence.data(host, "serial")
